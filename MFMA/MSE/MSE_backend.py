...
class MSE_backend(object):
    ...
    def fps_func(self,t,event):
        event.set()
        time.sleep(t)
        t = threading.Thread(target=self.fps_func,args=(t,event))
        t.start()
    def render_func(self,event):
        event.set()
        time.sleep(1.0/30.0)
        t = threading.Thread(target=self.render_func,args=(event,))
        t.start()  
    ...
    def _start_process(self,manager_dict,common_queue,data_queue):
        ...
        if self.fps != 0:
            ...
            fps_event.set()
            fps_t = threading.Thread(target=self.fps_func,args=(1.0/self.fps,fps_event))
            fps_t.start()
        if self.use_gui:
            render_event = threading.Event()
            render_event.set()
            render_t = threading.Thread(target=self.render_func,args=(render_event,))
            render_t.start()
        while True:
